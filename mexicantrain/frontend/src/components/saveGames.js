...
const AddGamePage = () => {
    ...
    const [scores, setScores] = useState(Array.from({ length: rowCount }, () => Array(14).fill(0))); // State to store scores
    ...
    useEffect(() => {
        ...
        // Update scores whenever rowCount changes
        setScores(prevScores => {
            const newScores = [...prevScores];
            newScores.push(Array(14).fill(0)); // Initialize scores for the new row
            return newScores;
        });
    }, [rowCount]);
    ...
    // Updates the list of selected players when a player is selected
